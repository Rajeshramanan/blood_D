...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($units < 1) {
        ...
        $stmt = $pdo->prepare("INSERT INTO blood_requests (requester_id, blood_group, units_required, urgency, hospital_name, location, state, contact_number, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'Pending')");
        if ($stmt->execute([$_SESSION['user_id'], $blood_group, $units, $urgency, $hospital, $location, $state, $contact_number])) {
            ...
            foreach ($donors as $donor) {
                ...
                $notif_msg = "URGENT: $hospital ($location, $state) needs $units units of $blood_group blood. Please check your Pending Requests.";
